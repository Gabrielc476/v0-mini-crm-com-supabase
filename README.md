...
O "Gargalo Mor" do nosso desenvolvimento rodou em torno de dois fatores centrais contemporâneos que se somaram para escalar a dificuldade:
* **Instabilidade da API Escolhida**: Durante iterações e debulhamentos ao vivo para a Edge Function, retornos incompletos e rate limits da API da LLM atrapalhavam os blocos de teste.
* **Defasagem no Vibe Coding e Documentações Atrasadas**: A utilização do maquinário assíncrono em bleeding-edge (particularmente na leitura das interfaces `await cookies()` adotadas no React 19/Next 15) esbarrou com o conhecimento preestabelecido de *AI-Code Generation Tools* que tentavam compilar lógicas em padrões antiquados do Next 13.
**A Solução**: Isso obrigou transições pesadas de debugar logs em tela, isolar e forçar tratamento manual de tipagem dos Server Components injetando refatoração direta e agressiva em cima do conhecimento autônomo da máquina, contornando quebras e criando blocos "Try/Catch" com alertas visuais amigáveis nos casos limite ou transbordamento da LLM.
